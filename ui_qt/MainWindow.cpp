...
void MainWindow::incarcaFilme(const std::vector<Film*>& filme) {
    ...
    for (Film* f : filme) {
        ...
        connect(card, &FilmCard::filmSelectat,
                this, &MainWindow::onFilmSelectat);
...
void MainWindow::onFiltrareSchimbata() {
    ...
    if (!q.isEmpty())
        rezultat = cinema.cautaFilme(q.toStdString());

    // Filtru tip
    if (tipFilter->currentIndex() == 1) {
        std::vector<Film*> tmp;
        for (auto* f : rezultat)
            if (f->getTip() == TipFilm::_2D) tmp.push_back(f);
        rezultat = tmp;
    } else if (tipFilter->currentIndex() == 2) {
        std::vector<Film*> tmp;
        for (auto* f : rezultat)
            if (f->getTip() == TipFilm::_3D) tmp.push_back(f);
        rezultat = tmp;
    }
...
void MainWindow::onFilmSelectat(int filmId) {
    ...
    dlg.exec();
    // Reincarca cardurile in caz ca disponibilitatea s-a schimbat
    incarcaFilme(cinema.getFilme());
}
